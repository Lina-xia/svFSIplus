...
void createModel(cpl1DType& cpl1D){
  ...
  auto outputFileName = cpl1D.outputFileName;
  auto* opts = cpl1D.opts;

  // MESSAGE
  outputFile << endl;
  outputFile << "Creating and Running Model ..." << endl;

  // CREATE MODEL MANAGER
  cvOneDModelManager* oned = new cvOneDModelManager((char*)opts->modelName.c_str());

  // CREATE NODES
  outputFile << "Creating Nodes ... " << endl;
  ...
  // CREATE JOINTS
  outputFile << "Creating Joints ... " << endl;
  ...
  // CREATE MATERIAL
  outputFile << "Creating Materials ... " << endl;
  ...
  // CREATE DATATABLES
  outputFile << "Creating Data Tables ... " << endl;
  ...
  // SEGMENT DATA
  outputFile << "Creating Segments ... " << endl;
...
void cvOneD::readModelFile(string& inputFile, cpl1DType& cpl1D, cvStringVec& includedFiles){
  
  auto outputFileName = cpl1D.outputFileName;
  auto* opts = cpl1D.opts;

//   ofstream outputFile(outputFileName, std::ios::app);  //在文件后面添加
  outputFile << endl;
  outputFile << "Reading file: " << inputFile.c_str() << "..." << endl;
...
//          opts->jointNode.push_back(atof(tokenizedString[2].c_str()));
        ...
        // outputFile << "Found Solver Options.\n");
...
// ====================
void cvOneD::readModel(ComMod& com_mod, string& inputFile, cpl1DType& cpl1D){
    ...
    // double dt,
    opts->timeStep = com_mod.dt;  //三维Time_step_size
    // long stepSize,
    opts->stepSize = com_mod.saveIncr; //三维Increment in saving solutions
    // long maxStep,
    opts->maxStep = com_mod.nTS;  //三维number_of_time_steps
    // // char* boundType
    // opts->boundaryType = BoundCondTypeScope::THREEDCOUPLING;

    // Read First File
    cvOneD::readModelFile(inputFile, cpl1DType& cpl1D, includedFiles);
